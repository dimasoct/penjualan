...
// Hitung total data
$total_sql = "SELECT COUNT(*) as total FROM kategori_barang";
...
$sql_kategori = $limit === 'all'
    ? "SELECT id_kategori, nama_kategori FROM kategori_barang"
    : "SELECT id_kategori, nama_kategori FROM kategori_barang LIMIT $start, $limit";
...
<main class="content">
    <div class="container-fluid p-0">
        <h1 class="h3 mb-3"><strong>Management</strong> Kategori Barang</h1>
        <div class="row">
            <div class="col-12 col-lg-12 col-xxl-12 d-flex">
                <div class="card flex-fill">
                    <div class="card-header">
                        <a class="btn btn-success col-auto mt-0 mb-2" href="add_kategori.php">Tambah Kategori</a>
                                    ...
                                    <?php if ($result_kategori->num_rows > 0): ?>
                                        ...
                                        <?php while ($row = $result_kategori->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td class="center"><?php echo htmlspecialchars($row['id_kategori']); ?></td>
                                                <td class="center"><?php echo htmlspecialchars($row['nama_kategori']); ?></td>
                                                <td class="center">
                                                    <a href="edit_kategori.php?id_kategori=<?php echo htmlspecialchars($row['id_kategori']); ?>" class="btn btn-primary btn-sm">
                                                        <i class="align-middle" data-feather="edit"></i>
                                                    </a>
                                                    <button class="btn btn-danger btn-sm" onclick="deleteKategori('<?php echo htmlspecialchars($row['id_kategori']); ?>')">
...
function deleteKategori(id_kategori) {
    if (confirm("Apakah Anda yakin ingin menghapus kategori ini?")) {
        ...
        xhr.open("POST", "delete_kategori.php", true);
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    alert("Gagal menghapus kategori: " + xhr.responseText);
                }
            }
        };

        xhr.send("id_kategori=" + encodeURIComponent(id_kategori));
    }
}
